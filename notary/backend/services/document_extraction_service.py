...
Supporte l'extraction de texte à partir de:
...
def get_file_category(filename: str, content_type: Optional[str] = None) -> Optional[str]:
    ...
    Returns:
        Catégorie ('pdf', 'word', 'text', 'markdown', 'image', 'audio') ou None
    """
...
class DocumentExtractionService:
    ...
    Méthodes d'extraction par type:
    - PDF: pypdf (rapide), Docling (avancé), OCR (scans)
    - Word: python-docx
    ...
    def _check_dependencies(self):
        ...
        try:
            ...
            logger.warning("pypdf non installé - extraction PDF limitée")

        try:
            import docx
            self._has_docx = True
        except ImportError:
            logger.warning("python-docx non installé - extraction Word limitée")

        try:
            from docling.document_converter import DocumentConverter
            self._has_docling = True
        except ImportError:
            logger.debug("Docling non installé - extraction avancée non disponible")

        try:
            import whisper
            self._has_whisper = True
        except ImportError:
            logger.debug("openai-whisper non installé - transcription audio limitée")

        try:
            from PIL import Image
            self._has_pillow = True
        except ImportError:
            logger.debug("Pillow non installé - extraction d'images limitée")
    ...
    async def extract(
        ...
        Args:
            ...
            content_type: Type MIME (optionnel)
            extraction_method: Méthode d'extraction ('auto', 'pypdf', 'docling', 'ocr')
        ...
        if not category:
            return ExtractionResult(
                success=False,
                error=f"Type de fichier non supporté: {file_path.suffix}"
            )

        # Router vers la méthode d'extraction appropriée
        if category == "pdf":
            ...
            return ExtractionResult(
                success=False,
                error=f"Catégorie non gérée: {category}"
            )
    ...
    async def _extract_pdf(
        ...
        # Par défaut: pypdf (rapide)
        if self._has_pypdf:
            return await self._extract_pdf_pypdf(file_path)

        return ExtractionResult(
            success=False,
            error="Aucune bibliothèque PDF disponible (installer pypdf ou docling)"
        )
    ...
    async def _extract_word(self, file_path: Path) -> ExtractionResult:
        ...
        if not self._has_docx:
            return ExtractionResult(
                success=False,
                error="python-docx non installé",
    ...
    async def _extract_image(self, file_path: Path) -> ExtractionResult:
        ...
        if not self._has_docling:
            return ExtractionResult(
                success=False,
                error="Docling non installé pour l'OCR d'images",
                extraction_method="ocr"
            )

        try:
            # Docling peut traiter les images
            from services.docling_service import get_docling_service

            service = get_docling_service(use_vlm=True)
            # Note: Docling peut traiter directement certains formats d'images
            # Pour une implémentation plus robuste, on pourrait convertir en PDF d'abord

            return ExtractionResult(
                success=False,
                error="L'extraction OCR d'images n'est pas encore implémentée",
                extraction_method="ocr"
            )
        except Exception as e:
    ...
    async def _transcribe_audio(
        ...
        try:
            ...
            # Charger le modèle (utiliser "base" pour la rapidité, "large" pour la qualité)
    ...
    async def _transcribe_audio_api(
        ...
        # - Autre service de transcription
