...
from models.audio_summary_models import (
    ...
    AudioSummaryImportScriptRequest,
...
# ============================================================================
# Script Import Endpoint
# ============================================================================

@router.post(
    "/api/audio-summaries/{summary_id}/import-script",
    response_model=dict,
    summary="Import markdown script"
)
async def import_script(summary_id: str, request: AudioSummaryImportScriptRequest):
    """
    Import an existing markdown script for an audio summary.

    This allows you to upload a script you've already created or edited,
    without needing to generate it with an LLM. The script will be parsed
    and stored, ready for audio generation.

    Supported formats:
    1. Scripts generated by this service (with *Voix: Name* annotations)
    2. Simple markdown with ## and ### headings (voices assigned automatically)
    """
    service = get_surreal_service()
    ...
    audio_summary_service = get_audio_summary_service()
    result = await audio_summary_service.import_script_from_markdown(
        summary_id=summary_id,
        markdown_content=request.markdown_content,
        voice_titles=request.voice_titles
    )

    if not result.get("success"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=result.get("error", "Erreur lors de l'import du script")
        )
...
# ============================================================================
# Audio-Only Generation Endpoint
# ============================================================================

@router.post(
    "/api/audio-summaries/{summary_id}/generate-audio",
    summary="Generate audio from existing script"
)
async def generate_audio_only(summary_id: str):
    """
    Generate audio from an existing script (without calling LLM).

    Use this endpoint when the script has already been generated and you only
    want to convert it to audio. This is useful for:
    - Re-generating audio with different settings
    - Using an existing script without consuming LLM credits
    ...
    if not summary.get("script_data"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Aucun script trouvé. Générez d'abord le script avec un modèle LLM."
        )
    ...
    async def event_stream():
        ...
        try:
            async for update in audio_summary_service.generate_audio_from_script(
                summary_id=summary_id
            ):
                yield f"data: {json.dumps(update)}\n\n"

        except Exception as e:
            logger.error(f"Error in audio generation stream: {e}")
            yield f"data: {json.dumps({'status': 'error', 'message': str(e)})}\n\n"
